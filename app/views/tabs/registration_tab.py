...
class StudentRegistrationForm(QWidget):
    ...
    def start_face_capture(self):
        ...
        try:
            ...
            if self.face_cascade.empty():
                QMessageBox.warning(
                    self, "Error", "Face detection classifier failed to load"
                )
                return
    ...
    def stop_face_capture(self, frame=None):
        ...
        if frame is not None:
            ...
            self.face_image_path = os.path.join(
                DATA_DIR, "student_faces", f"{student_id}_face.jpg"
            )
    ...
    def register_student(self):
        ...
        try:
            ...
            if not phone:
                QMessageBox.warning(
                    self, "Validation Error", "Phone number is required"
                )
                return
            ...
            try:
                ...
                QMessageBox.warning(
                    self,
                    "Database Error",
                    f"Could not register student: {str(db_error)}",
                )
...
class RegistrationTab(QWidget):
        ...
        try:
            ...
            if not camera_opened:
                QMessageBox.warning(
                    self,
                    "Camera Error",
                    "Could not open camera. No available camera found.",
                )
            ...
            QMessageBox.warning(
                self, "Camera Error", f"Could not start camera: {str(e)}"
            )
    ...
    def stop_camera(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Camera Error", f"Could not stop camera: {str(e)}"
            )
    ...
    def update_camera_frame(self):
        ...
        try:
            ...
            self.stop_camera()
            QMessageBox.warning(
                self, "Camera Error", f"Camera frame update failed: {str(e)}"
            )
        ...
        try:
            ...
            if len(faces) == 0:
                QMessageBox.warning(
                    self,
                    "Face Capture",
                    "No clear face detected. Please adjust position.",
                )
                return
            elif len(faces) > 1:
                QMessageBox.warning(
        ...
        try:
            ...
            for row, student in enumerate(students):
                ...
                self.students_table.setItem(
                    row, 0, QTableWidgetItem(student.get("student_id", ""))
                )
                self.students_table.setItem(row, 1, QTableWidgetItem(full_name))
                self.students_table.setItem(
                    row, 2, QTableWidgetItem(student.get("email", ""))
                )
                self.students_table.setItem(
                    row, 3, QTableWidgetItem(student.get("phone", ""))
                )
                self.students_table.setItem(
                    row, 4, QTableWidgetItem(student.get("date_of_birth", ""))
                )
                self.students_table.setItem(
                    row, 5, QTableWidgetItem(student.get("gender", ""))
                )
            ...
            QMessageBox.critical(
                self, "Database Error", f"Could not load students: {str(e)}"
            )
    ...
    def edit_student(self, row):
        ...
        self.date_of_birth_input.setDate(
            QDate.fromString(student_dict["date_of_birth"], "yyyy-MM-dd")
        )
    ...
    def delete_student(self, row):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(
                    self, "Deletion", f"Student {student_name} deleted successfully!"
                )

            except Exception as e:
                QMessageBox.critical(
                    self, "Deletion Error", f"Could not delete student: {str(e)}"
                )
    ...
    def show_student_details(self, row):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Could not show student details: {str(e)}"
            )
        return None
