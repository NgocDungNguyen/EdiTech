...
class StudentRegistrationForm(QWidget):
    ...
    def register_student(self):
        """
        Register student with face recognition
        """
        try:
            # Validate inputs
            student_id = self.student_id_input.text()
            ...
            if not all([student_id, first_name, last_name]):
                QMessageBox.warning(self, "Validation Error", "Student ID, First Name, and Last Name are required")
                return
            ...
            # Add face if captured
            if self.face_image_path and os.path.exists(self.face_image_path):
                success = self.face_recognition_manager.add_student_face(
                    student_id, 
                ...
                if not success:
                    QMessageBox.warning(
                        self, 
                        "Face Capture", 
                        "Student registered, but face capture failed"
                    )
...
class RegistrationTab(QWidget):
    ...
    def capture_face(self):
        ...
        if ret:
            try:
                ...
                # Generate unique filename
                filename = f"student_capture_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
                ...
                # Show captured face in a message box
    ...
    def register_student(self):
        ...
        # Get full name and split into first and last names
        ...
        student_dict = {
            ...
            'date_of_birth': self.date_of_birth_input.date().toString("yyyy-MM-dd"),
            'gender': self.gender_input.currentText(),
            'profile_image': self.captured_image if hasattr(self, 'captured_image') else None
        }

        try:
            # Use the updated database method
            student_id = self.db.add_student(student_dict)
            ...
            # Show success message
            QMessageBox.information(self, "Success", f"Student {full_name} registered successfully!")
