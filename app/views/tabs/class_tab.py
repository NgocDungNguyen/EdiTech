from PyQt6.QtWidgets import (
    ...
    QPushButton, QFrame, QFormLayout, QTableWidget, 
    QTableWidgetItem, QTabWidget, QDialog, QDialogButtonBox, 
    QTextEdit, QComboBox, QHeaderView, QGridLayout, QGroupBox, 
    QCheckBox, QTimeEdit, QSpinBox, QMessageBox, QSplitter, QAbstractItemView
)
...
class MultiDayScheduleWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.schedules_table.setHorizontalHeaderLabels(["Days", "Start Time", "End Time", "Actions"])
                ...
                try:
                    ...
                    for item in class_data_list:
                        if item is None:
                            ...
                            logging.warning(f"Unexpected data type: {type(item)} for item {item}")
                            processed_data.append(str(item))
                    ...
                    logging.info(f"Processed data types: {[type(x) for x in class_data]}")
    ...
    def view_class_details(self, index=None):
        ...
        try:
            ...
            if not class_data:
                ...
                QMessageBox.warning(self, "Not Found", f"No details found for class {class_id}")
                return
            ...
            logging.info(f"Found {len(schedule_results)} schedules for class {class_id}")
    ...
    def register_class(self):
        ...
        try:
            ...
            try:
                ...
                """, (
                    class_data['class_id'], class_data['name'], class_data['subject'], 
                    class_data['teacher'], class_data['room'], class_data['class_type'], 
                    class_data['description'], class_data['max_capacity']
                ))
...
class ClassDetailsDialog(QDialog):
    ...
    def populate_existing_data(self, class_data):
        ...
        if existing_schedules:
            ...
            for schedule in existing_schedules:
                ...
                self.schedule_widget.schedules_table.setItem(row, 0, QTableWidgetItem(days))
                self.schedule_widget.schedules_table.setItem(row, 1, QTableWidgetItem(start_time))
                self.schedule_widget.schedules_table.setItem(row, 2, QTableWidgetItem(end_time))
                
                # Add delete button
                delete_btn = QPushButton("Delete")
                delete_btn.clicked.connect(lambda _, r=row: self.schedule_widget.delete_schedule(r))
    ...
    def load_enrolled_students(self):
        ...
        try:
            ...
            for row, (student_id, full_name) in enumerate(students):
                ...
                remove_btn.clicked.connect(lambda _, sid=student_id: self.remove_student(sid))
    ...
    def add_student(self):
        ...
        try:
            ...
            if not self.class_id.text():
                QMessageBox.warning(self, "Error", "No class selected. Please select a class first.")
                return
            ...
            dialog = StudentSelectionDialog(existing_students=existing_students, parent=self)
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                if not selected_students:
                    QMessageBox.warning(self, "No Students Selected", "Please select at least one student to add.")
                    return
                ...
                # Show success message
                if added_count > 0:
                    QMessageBox.information(self, "Success", f"{added_count} student(s) added to class")
                else:
                    QMessageBox.warning(self, "No Students Added", "All selected students are already enrolled in this class.")
    ...
    def remove_student(self, student_id):
        ...
        try:
            ...
            if reply == QMessageBox.StandardButton.Yes:
                ...
                # Show success message
                QMessageBox.information(self, "Student Removed", "Student successfully removed from the class.")
    ...
    def save_class_details(self):
        ...
        try:
            ...
            # Close dialog
            QMessageBox.information(self, "Success", "Class details updated successfully!")
            self.accept()
...
class StudentSelectionDialog(QDialog):
    def __init__(self, existing_students=None, parent=None):
        ...
        self.students_table.setHorizontalHeaderLabels(["", "Student ID", "Name", "Email", "Gender"])
        
        # Make first column a checkbox
        self.students_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        self.students_table.setSelectionMode(QAbstractItemView.SelectionMode.NoSelection)
    ...
    def load_students(self):
        ...
        for row, student in enumerate(students):
            ...
            # Email
            self.students_table.setItem(row, 3, QTableWidgetItem(student.get('email', '')))
            
            # Gender
            self.students_table.setItem(row, 4, QTableWidgetItem(student.get('gender', '')))
